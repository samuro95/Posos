...
def get_QUEARO_annorations():
    """
    Get the medical categories annotations from QUEARO.
    """
...
def get_med_list(med_path='../data/meds.txt'):
    '''
    get a list of all the med names
    '''
    med_list=[]
...
def load_data(data_path, language = 'fr'):
    '''
    return the train, test sentences and corresponding classes
    '''
    train_sent = []
    y_train = []
    test_sent = []
...
def load_corrected_sentences(data_path):
    '''
    load corrected sentence already preprocessed with the function clean_sequences and previsouly saved
    '''
    train_sent = []
    test_sent = []
...
def build_dictionary(sequences):
    '''
    build the dictionary of all the words used in the datasets
    '''
    dict = {}
...
def encode_sequences(train_sent,test_sent,val_sent, y_train, y_val, maxlen = 50, resize = True):
    '''
    encode the tokens in the sentences in uniques integers
    '''
    tokenizer = Tokenizer()
